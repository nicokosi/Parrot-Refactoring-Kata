...
class ParrotTest extends \PHPUnit_Framework_TestCase {
  ...
  /** @test */
  function get_speed_of_european_parrot() {
    $parrot = new EuropeanParrot();
    $this->assertEquals(12.0, $parrot->getSpeed());
  }

  /** @test */
  function get_speed_of_african_parrot_with_one_coconut() {
    $parrot = new AfricanParrot(1);
    $this->assertEquals(3.0, $parrot->getSpeed());
  }

  /** @test */
  function get_speed_of_african_parrot_with_two_coconuts() {
    $parrot = new AfricanParrot(2);
    $this->assertEquals(0.0, $parrot->getSpeed());
  }

  /** @test */
  function get_speed_of_african_parrot_with_no_coconuts() {
    $parrot = new AfricanParrot(0);
    $this->assertEquals(12.0, $parrot->getSpeed());
  }

  /** @test */
  function get_speed_norwegian_blue_parrot_nailed() {
    $parrot = new NorwegianBlueParrot(1.5, true);
    $this->assertEquals(0.0, $parrot->getSpeed());
  }

  /** @test */
  function get_speed_norwegian_blue_parrot_not_nailed() {
    $parrot = new NorwegianBlueParrot(1.5, false);
    $this->assertEquals(18.0, $parrot->getSpeed());
  }

  /** @test */
  function get_speed_norwegian_blue_parrot_not_nailed_high_voltage() {
